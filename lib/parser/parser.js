...
const parser = new Parser({
  bnf: {
    ...
    let: [
      ['LET IDENTIFIER = expression NEWLINE', '$$ = yy.ast.let($2, null, $4)'],
      ['LET IDENTIFIER : typehint = expression NEWLINE', '$$ = yy.ast.let($2, $4, $6)'],
      ['LET IDENTIFIER : typehint NEWLINE', '$$ = yy.ast.let($2, $4, null)']
    ],
    assignment: [
    ...
    inline_array_elements: [
      ['inline_array_elements , subexpression', '$$ = $1.concat($3)'],
      ['subexpression', '$$ = [$1]']
    ],
    // REMAINING EXPRESSIONS
    // =========================================================================
    array_access: [
      ['IDENTIFIER [ expression ]', '$$ = yy.ast.arrayAccess($1, $3)']
    ],
    identifier: [
...
module.exports = function (input) {
  ...
  try {
    ...
    if (/Parse error on line/.test(error)) { // hijack lexer errors and fix position
      ...
      throw new Error(`Syntax error in line ${pos.row}, column ${pos.col}. Unexpected '${error.hash.token}'`)
    }
