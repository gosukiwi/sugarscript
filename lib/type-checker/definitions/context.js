...
class Context {
  ...
  getQueryType (parts, checkOne, accu) {
    ...
    if (accu) { // second+ call
      ...
      if (head.type === 'ARRAY_ACCESS') {
        head.index.forEach((index) => {
          if (checkOne(index).isnt('INTEGER')) throw new Error(`Array access must be INTEGER, '${index.type}' given`)
          type = type.value
        })
      }
      return this.getQueryType(parts.slice(1), checkOne, type)
    }
