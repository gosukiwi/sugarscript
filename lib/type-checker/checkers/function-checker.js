...
class FunctionChecker {
  check ({ node, checker, definitions }) {
    if (Object.prototype.hasOwnProperty.call(PRIMITIVES, node.name.toLowerCase()) || definitions.getBuiltInFunction(node.name) !== null) {
      throw new Error(`Function '${node.name}' already exists (${checker.file} at line ${node.position.row - 1})`)
    }
