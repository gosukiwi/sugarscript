...
class FunctionCallChecker {
  ...
  checkUserDefinedFunction ({ node, checker, definitions }) {
    ...
    if (func === null) throw new Error(`Tried to call ${name} but function was not yet defined (${checker.file} at line ${node.position.row - 1})`)
  ...
  checkPrimitive ({ node, checker, definitions }) {
    ...
    if (node.statement && !validator.statement) throw new Error(`'${name}' cannot be called as statement, at line ${node.position.row - 1}`)
    if (validator.params !== node.args.length) throw new Error(`Invalid number of arguments for '${name}', expected ${validator.params.length} got ${node.args.length}, at line ${node.position.row - 1}`)

    const args = node.args.map((node) => checker.checkOne({ node, definitions }))
    if (args.length !== validator.params) throw new Error(`Invalid number of arguments for '${name}', expected ${validator.params.length} got ${node.args.length}, at line ${node.position.row - 1}`)
  ...
  checkBuiltIn ({ node, checker, definitions }) {
    ...
    throw new Error(`Invalid arguments to ${name} (${checker.file} at line ${node.position.row - 1})`)
  }
