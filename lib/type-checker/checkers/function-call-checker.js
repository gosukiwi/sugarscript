...
class FunctionCallChecker {
  ...
  checkPrimitive ({ node, checker, definitions }) {
    ...
    if (!validate(...args)) throw new Error(`Invalid arguments to '${node.name}' (${args.join(', ')})`)
...
const PRIMITIVE_CHECKER = {
  array_length: {
    ...
    params: 1,
    statement: false,
    validate: function (p1) { return p1.is('ARRAY') }
  },
  array_insert: {
    type: new Type('VOID'),
    params: 2,
    statement: true,
    validate: function (p1, p2) { return p1.is('ARRAY') && p2.equals(p1.value) }
  },
  array_insert_at: { // array_insert_at(arr, index, value)
    type: new Type('VOID'),
    params: 3,
    statement: true,
    validate: function (p1, p2, p3) { return p1.is('ARRAY') && p2.is('INTEGER') && p3.equals(p1.value) }
  },
  array_insert_sorted: {
    type: new Type('VOID'),
    params: 2,
  ...
  array_remove_at: {
    ...
    params: 2,
    statement: true,
    validate: function (p1, p2) { return p1.is('ARRAY') && p2.is('INTEGER') }
  },
  array_sort: {
    type: new Type('VOID'),
    params: 1,
  ...
  array_find: {
    ...
    validate: function (p1, p2) {
      ...
      if (p1.value.isnt('UDT')) {
        if (p1.value.equals(p2)) return true
