...
class LetChecker {
  check ({ node, definitions, checker }) {
    ...
    if (valueType !== null && node.typehint !== null && !node.typehint.equals(valueType)) throw new Error(`Cannot assign ${valueType} to ${node.typehint} (${checker.file} at line ${node.position.row - 1})`)
