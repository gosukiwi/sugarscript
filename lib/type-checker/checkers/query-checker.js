class QueryChecker {
  check ({ node, definitions, checker }) {
    return this.getQueryType({ parts: node.parts, checker, definitions })
  }

  // private

  getQueryType ({ parts, definitions, checker, accu }) {
    ...
    if (accu) {
      ...
      return this.getQueryType({ parts: parts.slice(1), definitions, checker, accu: type })
    }
