...
class BinopChecker {
  check ({ node, checker, definitions }) {
    ...
    if (['AND', 'OR'].includes(node.name)) {
      if (!lhs.is('INTEGER')) throw new Error(`Incompatible types: The operator '${node.name}' does not work on ${lhs}`)
      if (!rhs.is('INTEGER')) throw new Error(`Incompatible types: The operator '${node.name}' does not work on ${rhs}`)
      return new Type('INTEGER')
    }
